  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cityResponse = await fetch('http://localhost:5000/api/images/bcharre');
        const cityData = await cityResponse.json();
        setCityData(cityData);

        // Fetch activity URLs
        const activityResponse = await fetch('http://localhost:5000/api/city/5/activity-urls');
