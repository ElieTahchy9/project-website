  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cityResponse = await fetch('http://localhost:5000/api/images/laqlouq');
        const cityData = await cityResponse.json();
        setCityData(cityData);

        // Fetch activity URLs
        const activityResponse = await fetch('http://localhost:5000/api/city/7/activity-urls');
